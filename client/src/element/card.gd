...
var _size: Vector2 = Vector2(200, 200)
...
#		ctx.draw_texture_rect(_texture, Rect2(Vector2.ZERO - _card_pivot, _card_size), false)
		...
		ctx.draw_string(config.font, position - Vector2(0, 16), "Bill Amstrong")
		...
		ctx.draw_rect(Rect2(position - Vector2(0, 56), Vector2(card_size.x, 42)), Color.black)
		ctx.draw_string(config.font, position - Vector2(0, 16), "3000")
		if highlight():
