...
#	var line_margin := Vector2(10, 10) 
	...
	if miroring:
		tabel.setup(Rect2(rect.position, line_size), null, card_x_indent, line_margin)
		...
		tabel.setup(Rect2(hand.position() + Vector2(0.0, line_size.y), line_size), null, card_x_indent, line_margin)
